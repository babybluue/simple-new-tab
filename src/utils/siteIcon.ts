 ...
 */
export function resolveSiteIcon(input: SiteIconInput): { logo?: string; favicon?: string } {
  const logo = input.logo || tryGetLogoForUrl(input.url)
  const favicon = getFavicon({ domain: input.domain, url: input.url, favicon: input.favicon }) || undefined
