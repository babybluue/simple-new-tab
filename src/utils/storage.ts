// 存储工具函数
import { getDefaultStarterQuickLinks } from './presets'
import { getPresetIconUrl, isAutoGeneratedFavicon } from './siteIcon'
import {
  getEffectiveTheme,
...
export const DEFAULT_QUICK_LINKS: QuickLink[] = (() => {
  const defaultsFromPresets = getDefaultStarterQuickLinks()
...
const normalizeQuickLink = (link: QuickLink): QuickLink => {
  const hostname = extractDomainFromUrl(link.url)
  const preset = getPresetIconUrl(link.url)
  const legacy = link as QuickLink & {
    logo?: string
  ...
  let customFavicon = link.customFavicon?.trim() || undefined
  const rawFav = legacy.favicon

  if (rawFav && !customFavicon) {
    const isChromeCache = rawFav.includes('chrome-extension:') && rawFav.includes('/_favicon/')
    const isAuto = isAutoGeneratedFavicon({ domain: hostname, url: link.url }, rawFav)
    if (!isAuto && !isChromeCache) {
      if (!(preset && rawFav === preset)) {
        customFavicon = rawFav
      }
    }
  }
...
export async function getQuickLinks(): Promise<QuickLink[]> {
  ...
  if (!stored.length) {
    ...
    // 轻量迁移：如果 normalizeQuickLink 清理/补齐了字段（例如去掉自动 favicon、剥离冗余 domain/default、补齐预设图标路径），
    ...
    const changed = stored.some((prev, idx) => {
      ...
      return (
        ...
        prev.customFavicon !== next.customFavicon ||
        (prev as QuickLink & { domain?: string }).domain !== undefined ||
        (prev as QuickLink & { default?: boolean }).default !== undefined ||
        (prev as QuickLink & { category?: string }).category !== undefined
      )
    })
    if (changed) {
      await chrome.storage.local.set({ quickLinks: normalized })
